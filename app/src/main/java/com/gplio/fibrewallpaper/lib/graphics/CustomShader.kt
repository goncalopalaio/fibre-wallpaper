...
open class CustomShader(protected val defaultVertexShaderCode: String, protected val defaultFragmentShaderCode: String, private val textureOes: Int = -1) {
    ...
    fun init(vertexShaderCode: String = defaultVertexShaderCode, fragmentShaderCode: String = defaultFragmentShaderCode): Boolean {
        val compiledProgram = createGlShaderProgram(vertexShaderCode, fragmentShaderCode)
        if (compiledProgram == -1) {
            return true
        }
    ...
    fun draw(
        ...
        if (viewProjectionMatrix != null) {
            GLES20.glUniformMatrix4fv(uniformViewProjectionLocation, 1, false, viewProjectionMatrix, 0)
        }
        for (shape in shapes) {
